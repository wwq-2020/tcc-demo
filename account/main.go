...
const (
	addr = "127.0.0.1:9002"
	dsn  = "root:devilsm8875@tcp(127.0.0.1:3306)/account"
)
...
func (s *server) try(w http.ResponseWriter, r *http.Request) {
	...
	if _, err := tx.Exec("update account set amount = amount - ? where id = ? and amount >= ?", bizData.Price, bizData.UserID, bizData.Price); err != nil {
		logrus.Error(err)
		return
	}

	if _, err := tx.Exec("insert into account_tmp(biz_id, amount) values(?,?)", bizData.OrderID, bizData.Price); err != nil {
		logrus.Error(err)
		return
	}
...
func (s *server) cancel(w http.ResponseWriter, r *http.Request) {
	...
	if _, err := tx.Exec("delete from account_tmp where biz_id = ?", bizData.OrderID); err != nil {
		logrus.Error(err)
		return
	}

	if _, err := tx.Exec("update account set amount = amount + ? where id = ?", bizData.Price, bizData.UserID); err != nil {
		logrus.Error(err)
		return
	}
